class BasicLoginGate {
    ...
    constructor(config) {
        ...
        if(config) {
            this.header = config.header ? config.header : "Register to";
    ...
    render() {
        ...
        if(this.dismissible) {
            rawHtml = rawHtml.replace('%%dismissButton%%', this.dismissButton);
        } else {
            rawHtml = rawHtml.replace('%%dismissButton%%', "");
        }
    ...
    _toggleNodeVisibility(element) {
        element.style.display = window.getComputedStyle(element).display === 'block' ? 'none' : 'block';
    }
